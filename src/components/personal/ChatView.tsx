      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 min-h-0" style={{ backgroundColor: "#fafafa" }}>
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <AgentMsg proactive time="08:00">
            ...
            <div className="text-sm text-gray-600 bg-indigo-50 rounded-xl px-4 py-3 border border-indigo-100 max-w-lg">
              ⚠️ <strong>Обрати внимание:</strong> Foxconn поднял цены на 3 позиции (8-12%). У тебя встреча
            ...
            <>
              ...
              {stateReached(chatState, "flight_done") && (
                <AgentMsg time="14:32">
                  ...
                  <button
                    onClick={() =>
                      onOpenContext({ type: "execution", title: "✈️ Детали поиска" })
                    }
            ...
            <>
              ...
              {chatState === "auto_create" && (
                <AgentMsg time="14:48">
                  ...
                  <div className="bg-white border-2 border-amber-200 rounded-xl overflow-hidden max-w-lg animate-fade-in">
                    <div className="px-4 py-2.5 bg-amber-50 border-b border-amber-100">
                      <span className="text-sm font-semibold text-amber-700">🔄 Новая автоматизация</span>
                    </div>
                    ...
                    <div className="px-4 py-3 border-t border-amber-100 flex gap-2">
                      <button
                        ...
                        className="px-4 py-2 bg-amber-500 text-white rounded-lg text-sm font-medium cursor-pointer hover:bg-amber-600"
                      >
                        ✓ Создать
                      </button>
              ...
              {chatState === "auto_done" && (
                <AgentMsg time="14:49">
                  <div className="text-sm text-gray-700">
                    ✅ Автоматизация создана! Первый запуск: завтра 07:30.
                  </div>
                  <AutoRunCard title="☀️ Утренняя сводка (тест)">
                    <div>💱 Курс: 1 CNY = 12.45₽ (▲0.3%)</div>
                    <div>🌤 Шэньчжэнь: 26°C, облачно</div>
                    <div>📋 Задачи (7): Закупки 3, Маркетинг 2, HR 2</div>
                    <div className="text-amber-700">⚠️ Тендер Q2 — дедлайн завтра, 25%</div>
                  </AutoRunCard>
                </AgentMsg>
              )}
            </>
      ...
      <div className="bg-white border-t border-gray-100 px-6 py-4 shrink-0">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-gray-50 rounded-2xl px-4 py-3 flex items-center border border-gray-200 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
              <input
                type="text"
                value={input}
            ...
            <button
              onClick={handleSend}
              className="w-10 h-10 bg-indigo-500 text-white rounded-xl flex items-center justify-center hover:bg-indigo-600 cursor-pointer shrink-0 text-base shadow-sm"
            >
              ↑
            </button>
          </div>
          <div className="flex gap-2 mt-2.5 flex-wrap">
            {[
              { id: "flights", icon: "🛫", label: "Рейсы" },
              { id: "workspace", icon: "📦", label: "Пространство" },
              { id: "expense", icon: "🧾", label: "Чеки" },
              { id: "blocked", icon: "🚫", label: "Блокировка" },
              { id: "auto", icon: "🔄", label: "Автоматизация" },
            ].map((a) => (
              <button
                key={a.id}
                onClick={() => handleQuickAction(a.id)}
                className="px-3 py-1.5 bg-gray-100 rounded-xl text-xs text-gray-500 cursor-pointer hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
              >
                {a.icon} {a.label}
              </button>
            ))}
